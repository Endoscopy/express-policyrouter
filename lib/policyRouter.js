...
var debug								 = require('debug')('policyRouter');
var Q                    = require('q');
...
class PolicyHandlerRegistry {
	...
	add(handler) {
		...
		debug('Register policy handler "' + handler.getName() + '"');
	}
	...
	remove(name) {
		...
		for (var i = this.pipeline.length; i >= 0; i--) {
			if (name === this.pipeline[i].getName()) {
				debug('Unregister policy handler "' + name  + '"');
...
class PreHookChainInvoker {
	...
	_done(error) {
		if (error) {
			return this.beginDeferred.reject(error);
		}
...
class PostHookChainInvoker {
	...
	_done(error) {
		if (error) {
			return this.endDeferred.reject(error);
		}
...
class MiddleWareProxy {
	...
	invoke(req, res, next) {
		...
		if (! res.resEnder) {
			var ender = new ResponseEnder(req, res, self.bindings, self.handlerRegistry);
			res.end.bind(res);
		...
		var nexter = new Nexter(req, res, next, self.bindings, self.handlerRegistry);
		var context = {
			req: req,
			res: res,
		...
		var preHookChainInvoker = new PreHookChainInvoker(context, self.handlerRegistry);
...
class Nexter {
	...
	next() {
		...
		var postHookChainInvoker = new PostHookChainInvoker(context, self.handlerRegistry);
 ...
 */
var PolicyRouterFacade = (function() {

  //================================	 
	//  Private variables defintions
	//================================
	 ...
	 * @private
	 */
	var _origRouter = null;

	/**
	 * Policy handlers
	 * @private
	 */
	var _handlerRegistry = new PolicyHandlerRegistry(); 
    ...
    /**
		...
		get(path, middleware, bindings) {
         ...
         throw new Error('Invalid parameter "' + middlwware+ '"');
			...
			var mwproxy = new MiddleWareProxy(middleware, bindings, _handlerRegistry);
      ...
      _origRouter.use(path, mwproxy.invoke.bind(mwproxy));
	    ...
	    throw new Error('Unsupported method for now');	
		}
