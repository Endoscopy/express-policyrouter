...
const debug = require('debug')('policyrouter');
const Q = require('q');
...
let PolicyRouterFacade = (function () {
  ...
  class PolicyRouter {
    ...
    get(path, middleware, bindings) {
      if (!path || ('string' !== typeof path && ! (path instanceof RegExp))) {
        throw new Error('Invalid parameter "' + path + '": "' + path + '"');
      }

      if (!middleware || 'function' !== typeof middleware) {
        throw new Error('Invalid parameter "' + middleware + '"');
      }
